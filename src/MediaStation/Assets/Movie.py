...
try:
    ...
    # We don't need a warning here since it is already issued in the bitmap decompression library.
    rle_c_loaded = False
...
#  - The second occurrence is usually completely transparent to show the key frame as it is, but this has a footer.
...
class MovieFrameFooter:
    ...
    def __init__(self, stream):
        ...
        if global_variables.version.is_first_generation_engine or \
            ...
            # It is theoretically possible for movies to have a variable framerate, but in reality all these are the same.
...
class MovieFrame(Bitmap):
    def __init__(self, chunk):
        ...
        # The footer right after this frame might not actually correspond to this frame.
        self.footer = None
...
# - Optional sound.
# - Zero or more "stills".
# - movie consists of a bunch of chunks.
class Movie(Animation):
    ...
    # Initialize a movie. This does not do any reading, as there are multiple parts to the movie that must be read separately.
    ...
    # \param[in] subfile - The subfile to read. The binary stream must generally be at the start of the subfile.
    ...
    # Currently doesn't handle keyframes that end in the middle of another frame, but that seems an unlikely occurrence.
        ...
        for index, frame in enumerate(self.frames):
            ...
            if frame.header.keyframe_end_in_milliseconds > timestamp:
                ...
                if current_keyframe is None or current_keyframe.header.index != frame.header.index:
                    ...
                    # Though maybe we could include them as some sort of "K1.bmp" filename to help with debugging.
    ...
    def export(self, root_directory_path, command_line_arguments):
        ...
        # TODO: Keyframe application is currently disabled becuase it is horribly inefficient and doesn't even work very well. It needs to be reworked.
